...
Before: $Before"
    ...
    ## Prompt to scan for large ISO files.
    Function PromptforScan {
        param(
            $ScanPath,
            $title = (Write-Host "Search for large files" -ForegroundColor Green),
            $message = (Write-Host "Would you like to scan $ScanPath for ISOs files?" -ForegroundColor Green)
        )
        ...
        switch ($prompt) {
           0 {
                Write-Host "Scanning $ScanPath for any large .ISO files per the Administrators request." -ForegroundColor Green
                Write-Verbose ( Get-ChildItem -Path $ScanPath -Include *.iso -Recurse -ErrorAction SilentlyContinue | 
